...
两个可选子功能在同一服务之上提供产品界面：`permissions` 会话投影单元（`src/types.ts` 声明该 key；单元以组合默认值为基础折叠三个全量值可调参数事件，并生成选择器视图，其中包含表内选项和仅作当前值的 `custom`）与 `/permission` 命令（不带参数调用时报告当前预设与表；预设参数经 `set` 切换）。该命令还自携 `complete` 提供方——预设名列表，当前项标记 `(current)`——因此交互式适配器通过命令注册表解析其参数菜单，无需任何界面侧特判。每个子功能仅在其注册表（`ctx.sessionProjections` / `ctx.commands`）被组合时激活。
