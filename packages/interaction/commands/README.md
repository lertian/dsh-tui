...
`ctx.commands.register(definition)` registers one lowercase command name, description, optional unstructured-input hint, optional argument-completion provider (`complete`), optional `recordInput` policy, and abortable handler. `complete` registers on the command itself — the Gemini `SlashCommand.completion(context, partialArg)` shape — so every command, including third-party plugin commands, can advertise argument candidates to interactive adapters without a per-surface special case. `recordInput` defaults to true; a command whose authoritative domain event owns the payload sets it to false so `command/run` omits `args` instead of duplicating the input. A registered command is available to every composed command adapter; a plugin that is incompatible with a deployment does not register there. A plain-context registration is global. A command-producing plugin mounted beneath `agent.ctx` declares its own `commands` injection and creates an exact agent-scoped definition; it shadows a global definition with the same name. This child-injection shape preserves the agent scope without making the core agent loop depend on a UI service. Duplicate names within one layer fail during registration. Every disposer is the exact Cordis effect disposer, and registration or removal notifies every `commands/change` observer so live adapters can refresh discovery; observer failures are logged and cannot veto the registry mutation or starve later observers.

`list(agent)` returns immutable, name-sorted descriptors after scoped shadowing. `find(agent, name)` returns the corresponding definition. `complete(agent, name, partialArg, signal)` resolves a command's argument candidates from its own `complete` provider — passing the live agent, the partial argument typed after the command name, and the UI's abort signal — and returns detached `{ value, label, description? }` rows, or `undefined` when the command is unknown or offers no provider; provider failures and malformed rows surface as thrown errors for the calling adapter to contain. Completion never logs, executes, or reaches the model. `execute(agent, line, signal)` uses `parseCommand()` and runs only a known command, returning the settled `CommandExecution` (the normalized result plus the lifecycle pairing `commandId`) or `undefined` for invalid syntax or unknown names. A resolved command's lifecycle is logged on the receiving agent's session as the log-only pair `command/run` (before the handler, with a minted `commandId`, the parser's structured name, the issuing `CommandSource`, and `args` unless `recordInput` is false) and `command/done` (at settlement, with the outcome kind and verbatim text; a successful result may also name an earlier non-command authoritative domain event through `sourceEventSeq`; a thrown or aborted handler settles as `kind: 'error'`). Admission misses log nothing. Both are direct standalone appends on the receiving agent's session: no turn wraps them, and persistence drains them through ordinary checkpoints and teardown.
...
Command discovery, completion, execution, and UI output add no model tokens. Explicit agent work scheduled by a command producer has the same token effect as the corresponding agent input.
...
- **Completion rows are free-form text only** — structured forms and typed-argument schemas remain command-owned parsing concerns; `complete` advertises `{ value, label, description? }` rows that the surface ranks and renders.
- **Cooperative side-effect cancellation** — dispatch stops awaiting on abort; handlers and completion providers must honor the signal to stop work that has already escaped into external systems.
